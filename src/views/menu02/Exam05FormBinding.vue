      ...
      <form v-on:submit.prevent="handleSubmit">
        ...
        <fieldset class="form-group row">
          ...
          <div class="col-sm-10">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" v-model="product.sex" value="woman" />
              <label class="form-check-label">Woman</label>
            </div>
          </div>
        </fieldset>
        <input type="submit" value="등록" class="btn btn-danger btn-sm mr-2" :disabled="!checkData" />
        <button type="button" class="btn btn-info btn-sm mr-2" @click="handleReset">재작성</button>
      </form>
    </div>
  </div>
</template>
...
const checkData = computed(() => {
  console.log("computed callback 실행");
  var result = product.name !== "" && product.company !== "";
  return result;
});
